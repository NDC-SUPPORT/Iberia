...
curl -XGET 'http://live-cloud.corp.iberia.es:9200/logstash-business-ndc-checkout-traces-*/_search?from=0' -H 'Content-Type: application/json' -u ndc_support:supp0rt#Kiban4 -d '{
	...
	"aggs": {
		"dataResults": {
			...
			"aggs": {
				"erroresTecnicosCodigo": {
					"terms": {
						"field": "kpi.airShoppingResponse.entity.errors.error.list.object.shortText.string.keyword",
						"size": 100,
						"order": {
							"_count": "desc"
						}
					...
					"aggs": {
						"erroresTecnicosDescripcion": {
							"terms": {
								"field": "kpi.airShoppingResponse.entity.errors.error.list.object.value.string.keyword",
								"size": 100,
								"order": {
									"_count": "desc"
								}
